# Awesome OpenClaw [![Awesome](https://awesome.re/badge.svg)](https://awesome.re)

<p align="center">
  <a href="README.md"><strong>English</strong></a> · <a href="README.ko.md">한국어</a>
</p>


> A curated list of awesome resources, tools, plugins, and guides for [OpenClaw](https://github.com/openclaw-korea/openclaw) — your personal AI assistant 🦞

## Contents

- [Official](#official)
- [Getting Started](#getting-started)
- [Channels & Integrations](#channels--integrations)
- [Skills & Plugins](#skills--plugins)
- [Deployment](#deployment)
- [Community](#community)
- [Contributing](#contributing)

## Official

- [OpenClaw](https://github.com/openclaw-korea/openclaw) - Main repository
- [openclaw.ai](https://openclaw.ai) - Official website
- [Discussions](https://github.com/openclaw-korea/openclaw/discussions) - Community Q&A

## Getting Started

- [Installation Guide](https://github.com/openclaw-korea/openclaw#getting-started) - Quick start setup
<!-- Add tutorials, video walkthroughs, blog posts here -->

## Channels & Integrations

Supported messaging channels:

| Channel | Status | Notes |
|---------|--------|-------|
| WhatsApp | Supported | Via Gateway |
| Telegram | Supported | Via Gateway |
| Slack | Supported | Via Gateway |
| Discord | Supported | Via Gateway |
| Google Chat | Supported | Via Gateway |
| Signal | Supported | Via Gateway |
| iMessage | Supported | Via BlueBubbles |
| Microsoft Teams | Supported | Via Gateway |
| Matrix | Supported | Extension |
| WebChat | Supported | Built-in |

## Skills & Plugins

<!-- Add community-built skills and plugins here -->

*Your skill here! Submit a PR to add it.*

## Deployment

<!-- Add deployment guides, Docker configs, cloud templates here -->

*Deployment guide coming soon.*

## Community

- [GitHub Discussions](https://github.com/openclaw-korea/openclaw/discussions) - Ask questions, share ideas
<!-- Add Discord, blog, YouTube, social links here -->

## Contributing

Contributions welcome! Read the [contribution guidelines](https://github.com/openclaw-korea/.github/blob/main/CONTRIBUTING.md) first.

To add a resource:
1. Fork this repo
2. Add your link to the appropriate section
3. Submit a PR

All entries must be:
- **Relevant** to OpenClaw
- **Working** (no dead links)
- **Described** in a few words
